...
const processPage = ({
  ...
  new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // HTTP Basic Auth (for e.g. htaccess-protected sites)
      ...
      // Cookies (e.g., WordPress logged-in cookies)
      if (Array.isArray(options.cookies) && options.cookies.length > 0) {
        // Attach cookies to this page; each cookie should have either `url`
        // or domain/path specified. If they don't, we default to the pageUrl.
...
const minimalcss = async (options) => {
  ...
  try {
    for (const pageUrl of urls) {
      ...
      if (!enableServiceWorkers) {
        // Use public CDP API instead of private _client
